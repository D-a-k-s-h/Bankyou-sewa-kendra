...
const RechargeSummary = ({rechargeSummary,setRechargeSummary}) => {
  ...
  return (
    <div className='fixed w-screen inset-0 z-[1000] font-michroma !mt-0 grid place-items-center overflow-auto bg-white bg-opacity-10 backdrop-blur-sm'>
        <div className='w-1/2 mx-auto flex flex-col text-lg gap-4 py-5 px-5 rounded-md bg-dark text-lightGray'>
            <div className='w-full flex flex-row items-center justify-between'>
                <p className='font-semibold text-green-500'>Payment Successfull</p>
                <button type='button' className='text-error' onClick={() => setRechargeSummary(null)}>X</button>
            </div>
            <div className='w-full px-5 py-3'>
                <p className='w-full text-center border border-lightGray p-2 uppercase font-semibold'>Payment Summary</p>
                ...
                <div className='flex flex-row justify-end items-center mt-4 gap-4'>
                    <button type='button' className='btn btn-success' onClick={handleDownload}>Print</button>
                    <button type='button' className='btn btn-outline text-lightGray' onClick={() => setRechargeSummary(null)}>Close</button>
                </div>
            </div>
        </div>
    </div>
